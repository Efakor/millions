...
public class Sale extends Transaction {
   ...
   * @throws IllegalStateException    if transaction is already committed or player
   *     doesn't own the share
   */
  @Override
  ...
  private void validateShareOwnership(Player player) {
    if (!player.getPortfolio().contains(getShare())) {
      throw new IllegalStateException(
          String.format("Player does not own share: %s", getShare().stock().getSymbol())
      );
    }
  }
}
